...
- **Unreleased (after 2.0.5)** — KMZ Start/Pause/Stop icons on the stored line; visible path is a tessellated `LineString` at height 0 (Earth Android `gx:Track` GPS-Z floated off the road at close zoom); Earth details include `Baro:` (ISA) and ExtendedData `baro` / GPS `alt`. Notes: [CHANGELOGS.md](../CHANGELOGS.md).
