...
- **Tap** on Google, OSM, and Turistautak opens the same menu on that point. The menu is tied to the geographic point and follows it when the camera moves. A new tap replaces the previous one. Drag and zoom stay. The menu and the distance target live on the Map tab, so the HUD distance survives a switch between Google and an offline map. The three actions run separately: address does not start the HUD, and distance does not show an address.
  ...
  - **GPS coordinate** — a card, not the HUD. Latitude and longitude are stacked, six decimals, dot as the decimal mark: `Lat: 47.497913`, then `Lon: 19.040236`. The copy icon puts `47.497913, 19.040236` on the clipboard (latitude first) so a map search finds the point. It stays until the next tap or **Close**.
  - **Address** — shown only when the map format can return ZIP, country, city, street, and house number together. Choosing it puts those fields on a card and leaves the HUD unchanged. None of the three current formats can, so the item stays hidden: a Google tap is only a coordinate; the OSM `.map` has a nearby street name and house number but no ZIP, country, or city tied to them; the Turistautak `.map` has no house numbers.
...
- **Search** (magnifying glass under My location) is shown only while an OSM or Turistautak map is in use. It looks up named places stored in that `.map`: city, town, village, hamlet, district, peak, statue, monument, landmark, house (`street` plus house number), building, street, and other named places. Layer switches do not hide a name that is in the file. At least 3 characters; accents are ignored (`Szobor` matches `szobor`). Up to 5 matches: the stronger name first, and the closer one first when the name matches equally well. Distance is from the live GPS fix, or from the map’s start point when there is no fix. Tap a row: **Navigate to &lt;name&gt;?** with the kind and the straight-line distance. **Yes** moves the camera there (zoom follows the kind) and sets the HUD distance target. **No** leaves the map. While logging, or while the whole track is kept on screen, the camera stays on that place until **My location** or the first pan, which follows the GPS fix again. The HUD distance keeps updating. Google Maps has no search, because those names are not in a downloaded file. The map in use is indexed in the background, including after the app leaves; places already stored can be searched, and the next launch continues from the saved cursor. If the 250 000 place cap is reached, search says only part of the map is indexed. Indexing waits while the battery or storage is low. Switching to another `.map` file clears the tap menu and the HUD distance; the distance survives a switch between Google and the offline map.
...
| Module    | Role                                                                                                                                                                            |
| --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `:engine` | Pure JVM: GNSS classification, skyplot projection, GPS altitude pick, baro/QNH, Kalman track filter, fix acceptance, speed-adaptive spacing, Douglas–Peucker, track stats, KML/KMZ, GPX 1.1, map HUD visibility, tap straight-line and coordinate labels, target pointer for the tap straight-line, Route-tab idle speeds, compass MAG/TRUE heading, elevation series, OSM file/camera/redraw, OSM render-option categories, map-visibility rules, track endpoints, fix-cloud buffer. JUnit tests live here. |
...
| ----------------------------------------------------------------- | ----------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Use GNSS only**                                                 | Yes (source)                  | **On:** `GPS_PROVIDER` satellite-chip positions (all GNSS constellations). **Off:** Play Services fused HIGH_ACCURACY. If GNSS only is on and GPS is off, there is no fused fallback.                                                                                                                                                                                                                 |
...
| [docs/DBSTRUCT-en.md](docs/DBSTRUCT-en.md)                                     | SQLite schema: track log `gtl.db` and place cache `map-search.db` (when the file is created, wiped, and indexed)   |
